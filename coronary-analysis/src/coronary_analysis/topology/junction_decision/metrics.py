...
def local_thickness_score(
    distance_map: np.ndarray | None, center: np.ndarray, radius: int
) -> float:
    values = sample_disk_values(distance_map, center, radius)
    return float(np.mean(values)) if values else 0.0


def max_thickness_score(
    distance_map: np.ndarray | None, center: np.ndarray, radius: int
) -> float:
    values = sample_disk_values(distance_map, center, radius)
...
def append_distance_value(
    values: list[float], distance_map: np.ndarray, row: int, col: int
) -> None:
...
def opposite_direction_cost(
    arm_a: dict[str, Any], arm_b: dict[str, Any], center: np.ndarray
) -> float:
...
def arm_direction_from_center(
    arm: dict[str, Any], center: np.ndarray, lookahead: int = 12
) -> np.ndarray:
    path = arm["path"]
    if len(path) < 2:
    ...
    return normalize_vec(
        np.asarray(path[index], dtype=float) - np.asarray(center, dtype=float)
    )
...
def arm_local_curvature(arm: dict[str, Any], n_points: int = 20) -> float:
    ...
    if len(path) < 4:
        return 1.0
    directions = [
        normalize_vec(path[index] - path[index - 1]) for index in range(1, len(path))
    ]
...
def intensity_profile_similarity(
    ...
    if common_len < 3:
        return 1.0
    return normalized_profile_distance(
        intensity_a[:common_len], intensity_b[:common_len]
    )


def sample_intensity_along_arm(
    image_gray: np.ndarray, arm: dict[str, Any], n_points: int
) -> np.ndarray:
    values: list[float] = []
...
def normalized_profile_distance(
    intensity_a: np.ndarray, intensity_b: np.ndarray
) -> float:
